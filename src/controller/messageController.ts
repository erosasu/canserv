...
function normalizeContactTarget(contact: any): string | null {
  ...
  if (typeof contact === 'object') {
    const fromCommonKeys = contact.phone ?? contact.to ?? contact.chatId;
    if (
      typeof fromCommonKeys === 'string' ||
      typeof fromCommonKeys === 'number'
    ) {
      const value = String(fromCommonKeys).trim();
      return value ? value : null;
    }

    if (typeof contact._serialized === 'string') {
      const value = contact._serialized.trim();
      return value ? value : null;
    }

    if (typeof contact.id === 'string' || typeof contact.id === 'number') {
      const value = String(contact.id).trim();
      return value ? value : null;
    }

    if (contact.id && typeof contact.id === 'object') {
      if (typeof contact.id._serialized === 'string') {
        const value = contact.id._serialized.trim();
        return value ? value : null;
      }
      if (typeof contact.id.user === 'string') {
        const value = contact.id.user.trim();
        return value ? value : null;
      }
    }

    if (contact.wid && typeof contact.wid === 'object') {
      if (typeof contact.wid._serialized === 'string') {
        const value = contact.wid._serialized.trim();
        return value ? value : null;
      }
      if (typeof contact.wid.user === 'string') {
        const value = contact.wid.user.trim();
        return value ? value : null;
      }
    }
  }
...
export async function sendMessage(req: Request, res: Response) {
  ...
  try {
    const results: any = [];
    // Asegurar que phone sea un array y tomar el primer elemento si es necesario
    let phoneArray: any[];
    if (Array.isArray(phone)) {
      phoneArray = phone.length > 0 ? phone : [];
    } else if (phone) {
      phoneArray = [phone];
    } else {
    ...
    for (const contato of phoneArray) {
      const contactString = normalizeContactTarget(contato);
      if (!contactString) continue;
      results.push(await req.client.sendText(contactString, message, options));
    }
...
export async function sendFile(req: Request, res: Response) {
  ...
  try {
    const results: any = [];
    // Asegurar que phone sea un array
    const phoneArray = Array.isArray(phone) ? phone : [phone];
    for (const contact of phoneArray) {
      const contactString = normalizeContactTarget(contact);
      if (!contactString) continue;
      results.push(
        await req.client.sendFile(contactString, pathFile, {
          filename: filename,
          caption: msg,
